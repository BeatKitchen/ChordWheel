/*
 * HarmonyWheel.tsx — v3.17.77 🎯 Legend INSIDE Wheel Container!
 * 
 * 🎯 v3.17.77 KEY FIX:
...
useEffect(() => {
  ...
  const [audioInitialized, setAudioInitialized] = useState(false); // ✅ v3.17.77: Track if audio is ready
  const [showAudioPrompt, setShowAudioPrompt] = useState(false); // ✅ v3.17.77: iOS audio prompt
  const audioEnabledRef = useRef(true); // Ref for MIDI callback closure
  const [audioReady, setAudioReady] = useState(false); // ✅ v3.17.77: Start false, set true when initialized
  ...
  const lastWedgeClickTimeRef = useRef<number>(0); // ✅ v3.17.77: Track click timing
  const wedgeClickFnRef = useRef<Fn | "">(""); // ✅ v3.17.77: Track which wedge was clicked
  ...
  // ✅ v3.17.77: Track window size - use 768px breakpoint (more standard)
  ...
  // ✅ v3.17.77: Initialize audio on first user interaction (mobile requirement)
  useEffect(() => {
    const initAudio = () => {
      const ctx = initAudioContext();
      if (ctx.state === 'suspended') {
  ...
  // ✅ v3.17.77: Load song from URL - Check on mount and URL changes
  ...
  useEffect(() => {
    ...
    console.log('🔍 Checking URL params:', { hasSongParam: !!songParam, url: window.location.href });
    ...
    if (songParam) {
      ...
      if (songData && typeof songData === 'object' && typeof songData.text === 'string') {
        ...
        // Parse on next tick
        setTimeout(() => {
          console.log('🎵 Calling parseAndLoadSequence');
      ...
      console.log('ℹ️ No song param in URL');
    }
  }); // No deps - runs every render until hasLoadedFromURL is true
  
  // ✅ v3.17.24: Button pulse animation when key pressed
  const [pulsingButton, setPulsingButton] = useState<string | null>(null);
  const [pulsingWedge, setPulsingWedge] = useState<Fn | "">(""); // ✅ v3.17.77: Visual feedback on click
  const [showKeyDropdown, setShowKeyDropdown] = useState(false);
  const [showTransposeDropdown, setShowTransposeDropdown] = useState(false);
  const [showSongMenu, setShowSongMenu] = useState(false);
  const [shareURL, setShareURL] = useState<string>('');
  const [showShareCopied, setShowShareCopied] = useState(false); // ✅ v3.17.77: Share feedback
  const [showShareModal, setShowShareModal] = useState(false); // ✅ v3.17.77: Share options modal
  ...
  const parseAndLoadSequence = ()=>{
    const APP_VERSION = "v3.17.77-harmony-wheel";
  ...
  const applySeqItem = (it: SeqItem)=>{
    ...
    if (it.kind==="chord" && it.chord){
      ...
      // ✅ v3.17.77 FIX: Convert sharps to flats for NAME_TO_PC lookup
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      // ✅ v3.17.77: Changed to Shift+comma/period (< >) to avoid editor conflict
  ...
  function detect(){
    ...
    // ✅ v3.17.77 FIX: Bm7b5 ABSOLUTE PRIORITY - must check before diatonic
    ...
    // ✅ v3.17.77 FIX: C#dim triad [1,4,7] → V/ii bonus (must check before diatonic)
    ...
    // ✅ v3.17.77 FIX: C#dim7 [1,4,7,10] → V/ii bonus (removed bass requirement for sequencer)
    ...
    if (performance.now() >= homeSuppressUntilRef.current){
      ...
      // ✅ v3.17.77 FIX #3: DEFENSIVE - Don't let bonus chords match diatonic
    ...
    return layout
      ...
      return (
        <g key={fn} 
           ...
           onPointerDown={(e)=>{
             // ✅ v3.17.77: Touch support - pointer events work for mouse + touch
             e.preventDefault(); // Prevent default touch behaviors
             
             // ✅ v3.17.77: Click-to-clear with timer - only clear if clicking same wedge after delay
             const now = Date.now();
             ...
             // ✅ v3.17.77: Visual pulse feedback
             setPulsingWedge(fn);
           ...
           onPointerEnter={(e)=>{
             // ✅ v3.17.77: Pointer events for touch + mouse
  ...
  const initAudioContext = () => {
    ...
    // ✅ v3.17.77: Resume audio context on mobile (required by iOS/Android)
  ...
  const playNote = (midiNote: number, velocity: number = 0.5, isChordNote: boolean = false) => {
    ...
    // ✅ v3.17.77: Reduced to prevent clipping (chords = multiple notes adding up)
  ...
  return (
    <div style={{
      ...
      MozUserSelect:'none',
      msUserSelect:'none',
      touchAction: 'pan-y' // ✅ v3.17.77: Allow vertical scrolling on background
    }}>
      {/* ✅ v3.17.77: iOS Audio Prompt with silent note trick */}
      {showAudioPrompt && (
        <div
          onClick={() => {
            const ctx = initAudioContext();
      ...
      <div style={{
        ...
        {/* ✅ v3.17.77: Legend - moved up to reduce overlap */}
        {isDesktop && (
          <div style={{
            position:'absolute',
            top:90,
        ...
        {/* BKS Logo Header with Emblem + Help Button - v3.17.77: High z-index */}
        <div style={{marginBottom:0, position:'relative', zIndex:10002, display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
          ...
          {/* Skill Wheel only - top right - v3.17.77: High z-index for clickability */}
          <div style={{
            display:'flex', 
        ...
        {/* Wheel - v3.17.77: Bigger on mobile, matches keyboard width */}
  ...
  {/* ✅ v3.17.77: Black backing circle - pointer-events none for scrolling */}
...
{/* -------- END BONUS BLOCK -------- */}
            ...
            <button
              ...
              style={{
                position: 'absolute',
                right: 40,
                bottom: isDesktop ? 120 : 60,  // ← v3.17.77: LOWER on mobile (was backwards!)
                width: 32,
                height: 32,
                padding: 0,
          ...
          return (
            <div style={{maxWidth: WHEEL_W, margin:'0 auto 0', marginTop: 25}}>
              ...
              <div style={{
                ...
                {/* Guitar Tab - v3.17.77: Always visible, scales on mobile */}
                <div style={{
                  border:'1px solid #374151',
                  borderRadius:8,
                    background:'#0f172a',
                ...
                <div style={{marginBottom: 6, display:'flex', gap:8, alignItems:'stretch', maxWidth:'100%', overflow:'hidden'}}>
                  <textarea
                    ...
                    style={{
                      ...
                      fontFamily:'ui-sans-serif, system-ui',
                      resize:'vertical',
                      fontSize: isDesktop ? 12 : 16, // ✅ v3.17.77: 16px on mobile prevents iOS zoom
                  ...
                  {/* ✅ v3.17.77: Vertical button stack - Load & Share */}
                  <div style={{display:'flex', flexDirection:'column', gap:8}}>
                    {/* Load/Menu Button */}
                    <div style={{position:'relative'}}>
                      <button 
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      if (newState) {
                        ...
                        // ✅ v3.17.77: Play silent note to fully unlock iOS audio in iframe
                        const osc = ctx.createOscillator();
                        const gain = ctx.createGain();
...
// HarmonyWheel v3.17.77 - Legend inside wheel container (same stacking context as transform)

// EOF - HarmonyWheel.tsx v3.17.77