...
export default function HarmonyWheel(){
  ...
  // 🚀 v4.0.0 Feature Flag
  // const USE_NEW_ENGINE = true; // No longer needed - always using new engine
...
useEffect(() => {
  ...
  // ✅ Fix iframe scroll issue - prevent body overflow when embedded
  useEffect(() => {
    const isInIframe = window.parent && window.parent !== window;

    if (isInIframe) {
      // Set overflow hidden on body and html to prevent scroll
  ...
  useEffect(() => {
    ...
    if (songData && typeof songData === 'object' && typeof songData.text === 'string') {
      ...
      if (cleanText.startsWith('{') && cleanText.endsWith('}')) {
        console.warn('⏺️š ï¸ JSON object detected in text field, attempting to extract...');
        try {
  ...
  const [rhythmEnabled, setRhythmEnabled] = useState(false); // ✅ v4.5.2: Start with rhythm disabled so wedge clicks work immediately
        ...
        setTimeout(() => {
          ...
          if (allNotesReleased && midiLatchTimeoutRef.current === null) {
            ...
            const timerId = window.setTimeout(() => {
              console.log('⏺️° TIMEOUT FIRING - clearing everything');
              ...
              console.log('⏺️±ï¸ MIDI latch timeout - cleared display and keyboard highlights');
            }, 10000);
            
            midiLatchTimeoutRef.current = timerId;
            console.log('⏺️±ï¸ MIDI latch timer started - 10s until clear, timerId:', timerId);
          }
        }, 50);
    ...
    const fetchCalendarEvents = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error('🗓️ ⏺️Œ Teamup API error:', response.status, errorText);
        ...
        if (tickerEvents.length > 0) {
          ...
          console.log('🗓️ 📊 Ticker events array:', tickerEvents.map((e, i) => `[${i}] ${e.isLive ? '🔴 LIVE' : '⏺️'} "${e.text}"`));
        ...
        console.warn('⏺️š ï¸ Teamup fetch failed, using fallback events:', error);
    ...
    const items: SeqItem[] = rawTokens.map(tokenObj => {
      ...
      if (match) {
        ...
        if (degree !== undefined) {
          ...
          console.log('[PARSER] ⏺️Œ Failed to convert roman numeral - degree undefined');
        }
      }
  ...
  const goToStart = () => {
    ...
    if (sequence.length > 0) {
      ...
      if (startIdx < sequence.length) {
        ...
        // ⏺️Œ Don't play on rewind - just position
  ...
  const applySeqItem = (it: SeqItem): number[] => {
    ...
    if (it.kind==="chord" && it.chord){
      ...
      if (!match) {
        console.warn('⏺️š ï¸ Could not parse chord:', chordName);
        return [];
      }
      ...
      if (rootPc === undefined) {
        console.warn('⏺️š ï¸ Unknown root:', root, 'Available keys:', Object.keys(NAME_TO_PC));
        return [];
      }
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      if (performanceModeRef.current) {
        ...
        if (fn) {
          ...
          if (rhythmEnabledRef.current && chordNotes && chordNotes.length > 0) {
            ...
            console.log('⏺️¸ï¸ Rhythm disabled - not starting loop');
          } else {
            console.warn('⏺️š ï¸ No latched notes to play rhythm with');
          }
      ...
      if (e.key === ' ') {
        ...
        // ✅ Close performance mode if open
        if (performanceModeRef.current) {
          setPerformanceMode(false);
          performanceModeRef.current = false;
        }

        // ✅ Close song menu if open
        setShowSongMenu(false);
      ...
      // Legacy arrow navigation (still work)
        ...
        setLatchedAbsNotes([]); // ✅ Clear keyboard highlights
        lastInputWasPreviewRef.current = false; // ✅ Clear preview flag
        console.log('⏺️Œ MIDI latch manually cleared with X key');
      }
    };
  ...
  const previewChordByName = (chordName: string) => {
    ...
    if (fn) {
      ...
      if (!pcs || pcs.length === 0) {
        console.warn('⏺️š ï¸ preview.chordPcsForFn returned empty for:', fn, 'falling back to CHORD_DEFINITIONS');
  ...
  // ============================================================================
  // OLD detect() function REMOVED (1,326 lines)
  // Backed up to: OLD_detect_function_BACKUP.txt
  // New engine (detectV4) is now exclusively active
  // To revert: Restore from backup file or git
  // ============================================================================
      ...
      return (
        <g key={fn} 
           style={{
             touchAction: 'none', 
             cursor: 'pointer', 
             pointerEvents: 'auto'  // ⏺️š ï¸ CRITICAL: Must be 'auto' when parent SVG has pointerEvents:'none'
           }}
           onPointerDown={(e)=>{
             ...
             if (!ctm) {
               console.warn('⏺️š ï¸ getScreenCTM() returned null, using fallback');
           ...
           onPointerEnter={(e)=>{
             ...
             if (e.buttons === 1 && wedgeHeldRef.current && currentHeldFnRef.current !== fn) {
               ...
               const ctm = svg.getScreenCTM();
               if (!ctm) {
                 console.warn('⏺️š ï¸ CTM null in onMouseEnter');
                 previewFn(fn, true);
                 return;
               }
               const svgP = pt.matrixTransform(ctm.inverse());
           ...
           onPointerMove={(e)=>{
             ...
             const ctm = svg.getScreenCTM();
             if (!ctm) {
               console.warn('⏺️š ï¸ CTM null in onMouseMove');
               return;
             }
             const svgP = pt.matrixTransform(ctm.inverse());
  ...
  const previewFn = (fn:Fn, include7thOverride?: boolean)=>{
    ...
    if (chordDef) {
      ...
      console.warn('⏺️š ï¸ Function not in chord table, using fallback:', fn);
  ...
  const initAudioContext = () => {
    ...
    if (audioContextRef.current.state === 'suspended') {
      ...
      audioContextRef.current.resume().then(() => {
        ...
        console.error('⏺️Œ Failed to resume audio context:', err);
      });
    }
  ...
  const handleShareSong = () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url)
        ...
        .catch((err) => {
          console.error('⏺️Œ Clipboard API failed:', err);
          // Fallback: try execCommand
          fallbackCopyToClipboard(url);
        });
    } else {
      // Fallback for older browsers
      console.log('⏺️š ï¸ navigator.clipboard not available, using fallback');
      fallbackCopyToClipboard(url);
    }
  };
  ...
  const fallbackCopyToClipboard = (text: string) => {
    ...
    try {
      ...
      console.log(successful ? '✅ Copied via execCommand' : '⏺️Œ execCommand failed');
      if (successful) {
        setShareURL(text);
        setShowShareCopied(true);
        setTimeout(() => setShowShareCopied(false), 3000);
      ...
      console.error('⏺️Œ Fallback copy failed:', err);
        ...
        <div className="relative"
          ...
          <div style={{...wrapperStyle, position:'relative', zIndex:10}}>
            <svg width="100%" height="100%" viewBox={`0 0 ${WHEEL_W} ${WHEEL_H}`} className="select-none" style={{
              ...
              maxHeight:'100%', 
              touchAction:'pan-y', 
              pointerEvents:'none'  // ⏺️š ï¸ CRITICAL: Allows clicks to pass through to buttons underneath (wheel overlaps with marginTop:-30)
            }}>
          ...
          return (
            <div style={{
                ...
                <div style={{
                  ...
                  {showSequencerDisplay && sequence.length > 0 && songTitle ? (
                    ...
                    <div style={{
                      ...
                      whiteSpace:'nowrap',
                      width:'100%',
                      userSelect: 'auto',  // ✅ Allow text selection for links
                      WebkitUserSelect: 'auto',
                      pointerEvents: 'auto',  // ✅ Allow clicks on links
                      touchAction: 'auto',  // ✅ Override parent's pan-y to allow link clicks
                      cursor: 'default',
                      zIndex: 10,  // ✅ Ensure banner is above any overlays
              ...
              <div style={{
                ...
                pointerEvents: 'auto'  // ⏺️š ï¸ CRITICAL: Force clickability even when wheel SVG overlaps (marginTop:-30)
              }}>
              ...
              <div style={{marginTop: 6, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap'}}>
                ...
                {/* Transpose Bypass - v3.5.0: Toggle to temporarily disable - Always visible in expert mode */}
                {skillLevel === "EXPERT" && (
                  <button
                    ...
                    style={{
                      padding:'6px 10px',
                      border: transposeBypass ? '1px solid #6B7280' : (transpose === 0 ? '1px solid #374151' : '2px solid #10B981'),
                      borderRadius:8,
                      background: transposeBypass ? '#111' : (transpose === 0 ? '#111' : '#1a3a2a'),
                      color: transposeBypass ? '#6B7280' : (transpose === 0 ? '#4B5563' : '#10B981'),
                      cursor: transpose === 0 ? 'not-allowed' : 'pointer',
                      fontSize:11,
                      fontWeight: transposeBypass ? 400 : (transpose === 0 ? 400 : 600),
                      opacity: transpose === 0 ? 0.5 : 1
                    }}
                    title={transpose === 0 ? "Transpose bypass (inactive - no transpose set)" : (transposeBypass ? "Resume transpose" : "Bypass transpose (temporary disable)")}
                  >
                    {transposeBypass ? 'TR OFF' : 'TR ON'}
                  </button>
                )}
                
                {/* Play in C - Transpose to C (capo analogy) - Toggleable */}
                {baseKey !== 'C' && skillLevel === "EXPERT" && (
                  <button
                    onClick={() => {
                      ...
                      // ✅ Toggle: if already transposed to C, turn it off
                      if (transpose === transposeAmount && !transposeBypass) {
                        setTranspose(0);
                        setTransposeBypass(false);
                      } else {
                        // Engage transpose to C
                    ...
                    style={{
                      padding:'6px 10px',
                      border: (transpose === ((NAME_TO_PC[baseKey] || 0) === 0 ? 0 : (12 - (NAME_TO_PC[baseKey] || 0)) % 12) && !transposeBypass)
                        ? '2px solid #60A5FA'  // Active: thicker border
                        : '1px solid #60A5FA',  // Inactive: thin border
                      borderRadius:8,
                      background: (transpose === ((NAME_TO_PC[baseKey] || 0) === 0 ? 0 : (12 - (NAME_TO_PC[baseKey] || 0)) % 12) && !transposeBypass)
                        ? '#1e3a5f'  // Active: darker blue background
                        : '#111',    // Inactive: dark background
                      color:'#60A5FA',
                      cursor:'pointer',
                      fontSize:11,
                      opacity: (transpose === ((NAME_TO_PC[baseKey] || 0) === 0 ? 0 : (12 - (NAME_TO_PC[baseKey] || 0)) % 12) && !transposeBypass) ? 1 : 0.7
                    }}
                    title={`Transpose to C (like a capo on fret ${NAME_TO_PC[baseKey] || 0}) - Click to toggle`}
                  >
                    🎹 Play in C
                  </button>
                )}
              ...
              {skillLevel === "EXPERT" &&  (
                <div style={{display:'flex', gap:8, alignItems:'center', marginTop:6, marginBottom:8, flexWrap:'wrap'}}>
                  ...
                  {/* 1. Play/Stop - GREEN for ▷, RED for ⏺️–  - FIRST BUTTON */}
                  <button 
                  ...
                  >
                    {stepRecord ? '⏺️ Recording' : '⏺️ Step Record'}
                  </button>
                </div>
              )}
